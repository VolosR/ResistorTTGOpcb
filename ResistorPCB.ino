...
int Tfirst=first;
int Tsecon=secon;
int Tmultiplier=multiplier;
int Ttolerance=tolerance;
...
void loop() {
   ...
   first=map(analogRead(2),0,4095,0,9);
   secon=map(analogRead(15),0,4095,0,9);
   multiplier=map(analogRead(13),0,4095,0,11);
  ...
  if(digitalRead(0)==0)
  {
    if(deb1==0)
    {
     ...
     if(first!=Tfirst || secon!=Tsecon || multiplier!=Tmultiplier || tolerance!=Ttolerance)
    {
    
     
    
    
         tft.fillCircle(16,(Tfirst*13)+5,2,TFT_BLACK);
     
       tft.fillCircle(16,(first*13)+5,2,TFT_WHITE);
     
          
              tft.fillCircle(38,(Tsecon*13)+5,2,TFT_BLACK);
         
         tft.fillCircle(38,(secon*13)+5,2,TFT_WHITE);
           

            
              tft.fillCircle(188,(Tmultiplier*11)+5,2,TFT_BLACK);
         
          tft.fillCircle(188,(multiplier*11)+5,2,TFT_WHITE);
           

           
              tft.fillCircle(61+(Ttolerance*34),133,2,TFT_BLACK);
